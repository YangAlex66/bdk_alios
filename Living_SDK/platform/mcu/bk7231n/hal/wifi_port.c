...
static void ApListCallback(hal_wifi_scan_result_t *pApList);
static void ApListAdvCallback(hal_wifi_scan_result_adv_t *pApAdvList);
...
{
	bk_wifi_get_mac_address((char*)mac);
}
...
static int get_ip_stat(hal_wifi_module_t *m, hal_wifi_ip_stat_t *out_net_para, hal_wifi_type_t wifi_type)
{
	...
	os_memset(&ip_status, 0, sizeof(IPStatusTypedef));
	...
	os_memcpy(out_net_para->mac, ip_status.mac, sizeof(ip_status.mac));
...
static void scan_cb(void *ctxt, void *user)
{
	...
	apList.ap_list = NULL;
	sr.ApList = 0;
	if (wlan_sta_scan_result(&sr)) {
		/* cannot get scan result from wpas*/
		apList.ap_num = 0;
	} else {
		apList.ap_num = sr.ApNum;
	}

	if (apList.ap_num > 0) {
		apList.ap_list = (ap_list_t*)os_malloc(sizeof(*apList.ap_list) * apList.ap_num);
		os_memset(apList.ap_list, 0, sizeof(*apList.ap_list)*apList.ap_num);
		for (i = 0; i < sr.ApNum; i++) {
			os_memcpy(apList.ap_list[i].ssid, sr.ApList[i].ssid, 32);
			apList.ap_list[i].ap_power = sr.ApList[i].ApPower;
		}
	}
...
static void scan_adv_cb(void *ctxt, void *user)
{
	...
	if (apList.ap_num > 0) {
		...
		for (i = 0; i < sr.ApNum; i++) {
			os_memcpy(apList.ap_list[i].ssid, sr.ApList[i].ssid, 32);
			apList.ap_list[i].ap_power = sr.ApList[i].ApPower;
			os_memcpy(apList.ap_list[i].bssid, sr.ApList[i].bssid, 6);
			apList.ap_list[i].channel = sr.ApList[i].channel;
			apList.ap_list[i].security = sr.ApList[i].security;
		}
	}
...
{
    return bk_wlan_stop(BK_STATION);
}

static int suspend_soft_ap(hal_wifi_module_t *m)
{

    return bk_wlan_stop(BK_SOFT_AP);
}
...
int wlan_set_listeninterval(hal_wifi_module_t *m, uint8_t listen_interval)
{
	return 0;
}

int wlan_enter_powersave(hal_wifi_module_t *m, uint8_t recvDTIMs)
{
...
int wlan_exit_powersave(hal_wifi_module_t *m)
{
...
#if (WIFI_CONFIG_SUPPORT_LOWPOWER > 0)
	.set_listeninterval = wlan_set_listeninterval,
	.enter_powersave    = wlan_enter_powersave,
	.exit_powersave     = wlan_exit_powersave,
#endif
};
